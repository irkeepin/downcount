...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { name: @target.name, slug: @target.slug, target_date: @target.target_date } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { name: @target.name, slug: @target.slug, target_date: @target.target_date } }
